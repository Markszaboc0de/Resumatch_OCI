...
# --vm-bytes 15%: Allocate 15% of available memory. Kept low on purpose — the web workers
#   preload large ML models (bge-m3 ~2GB + GLiNER), so reserving 40% here risked OOM kills.
#   15% is enough to keep the instance "active" against Oracle's idle-reclaim.
# --vm-keep: Hold the memory instead of constantly reallocating (prevents memory bus thrashing)
ExecStart=/usr/bin/stress-ng --cpu 0 --cpu-load 40 --vm 1 --vm-bytes 15% --vm-keep
Restart=always
RestartSec=60
